...
export default function TeacherNotebookPanel({
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries
      ...
      .filter((entry) => (query ? entrySearchText(entry).includes(query) : true));
  ...
  async function removeWordDraft(draftId: string) {
    ...
    if (error) {
      setMessage(error.message ?? "Could not remove word.");
      return;
    }

    setWordDrafts((prev) => prev.filter((draft) => draft.id !== draftId));
    setSelectedWordIds((prev) => prev.filter((id) => id !== draftId));
  }
  ...
  async function copyWords(words: WordDraft[]) {
    const text = words.map((draft) => draft.surface.trim()).filter(Boolean).join("\n");
    if (!text) {
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="rounded-full bg-stone-950 px-3 py-1.5 text-xs font-black text-white"
          >
            Copy selected
          </button>
          <button
            type="button"
            onClick={() => void copyWords(wordDrafts)}
            className="rounded-full border border-stone-200 bg-white px-3 py-1.5 text-xs font-black text-stone-700"
          >
            Copy all
          </button>
          <Link
            href={userBookId ? `/vocab/bulk?userBookId=${encodeURIComponent(userBookId)}` : "/vocab/bulk"}
          ...
          <div className="space-y-2">
            {wordDrafts.map((draft) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => void removeWordDraft(draft.id)}
  ...
  return (
    <aside className={`${panelClasses} flex max-h-[calc(100vh-2rem)] flex-col overflow-hidden`}>
      <div className="shrink-0">
        ...
        {wordCaptureEnabled ? (
          <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3">
            {wordListOpen ? (
              <div className="mb-3 max-h-72 overflow-y-auto rounded-2xl border border-amber-200 bg-white p-3 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={saveEntry} className="rounded-2xl border border-stone-200 bg-stone-50 p-3">
              <div className="grid gap-3">
                ...
                {activeEntryType !== "note" && activeEntryType !== "translation" ? (
