// Teacher Manual Book Flags Queue
//
"use client";
...
import { requireSuperTeacher } from "../_shared/bookAttentionHelpers";
...
export default function TeacherBookFlagsPage() {
  const searchParams = useSearchParams();
  const backLink = getTeacherBackLink(searchParams.get("from"));

  const [loading, setLoading] = useState(true);
  const [message, setMessage] = useState("");
  const [flags, setFlags] = useState<ManualBookFlag[]>([]);
    ...
    try {
      ...
      setFlags(
        ((alerts ?? []) as any[]).map((alert) => {
          const book = alert.user_book_id ? bookByUserBookId.get(alert.user_book_id) : null;
          return {
            id: alert.id,
            user_book_id: alert.user_book_id ?? null,
    ...
    <main className="mx-auto max-w-5xl px-4 py-8">
      ...
      <section className="mt-4 rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-2 text-3xl font-black text-stone-900">
          {flags.length} manual book flag{flags.length === 1 ? "" : "s"}
        </h1>
        <p className="mt-2 text-sm leading-6 text-stone-600">
          Book Hubs manually sent to the review queue.
        </p>
      </section>

      {message ? <p className="mt-4 text-sm text-amber-700">{message}</p> : null}
      {loading ? <p className="mt-6 text-sm text-stone-500">Loading book flags...</p> : null}
