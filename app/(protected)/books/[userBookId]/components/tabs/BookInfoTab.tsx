        ...
        <div className="space-y-4">
          ...
          {(book.translator || book.translator_image_url || isEditingPeople) && (
            <div className="space-y-2">
              {isEditingPeople ? (
                <BookInfoRecordSearchPanel
                  label="Search existing translator"
                  searchValue={translatorSearch}
                  onSearchChange={(value) => {
                    setTranslatorSearch(value);
                    setSelectedTranslatorId(null);
                    setTranslatorSearchError(null);
                  }}
                  ...
                  onSelectResult={handleSelectTranslator}
                  noResultsMessage="No matching translator found in saved people records yet."
                  selectedLabel="Linked to person record"
                  hasSelectedRecord={Boolean(selectedTranslatorId)}
                  onClearSelected={clearSelectedTranslator}
                  canCreateSharedRecords={canCreateSharedRecords}
                  requireSharedRecord={requireSharedTranslatorRecord}
                  requireSharedRecordMessage="New shared translator record will be required on save."
                  createNewMessage="Don&apos;t see the right match? Create a new shared translator record from this search, then finish the details below."
                  createNewButtonLabel="Create New Translator"
                  onCreateFromSearch={startNewTranslatorFromSearch}
                />
              ) : null}
