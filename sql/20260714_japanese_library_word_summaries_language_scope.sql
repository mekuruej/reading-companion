-- Keep Japanese Library Study summaries scoped to Japanese saved words.
--
-- English Reader rows live in user_book_words too, but Japanese Library Review,
-- Ability Check, Word Sky, and study colors read user_library_word_summaries.
-- These summary functions therefore treat legacy NULL target_language_code rows
-- as Japanese and exclude all non-Japanese rows from the summary source.
  ...
  where user_id = p_user_id
    ...
    and not exists (
      ...
      where ub.user_id = p_user_id
        and coalesce(ubw.target_language_code, 'ja') = 'ja'
...
-- Remove derived summary rows that have no Japanese/legacy-Japanese backing row.
delete from public.user_library_word_summaries s
where not exists (
  select 1
  from public.user_book_words ubw
  join public.user_books ub on ub.id = ubw.user_book_id
  where ub.user_id = s.user_id
    and coalesce(ubw.target_language_code, 'ja') = 'ja'
    and public.library_study_identity_key(ubw.surface, ubw.reading) = s.study_identity_key
);
