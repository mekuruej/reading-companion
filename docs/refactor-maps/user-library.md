...
* What stays in `page.tsx`:
  ...
  * filtering logic
  * sorting logic
* Category: presentational UI with controlled inputs.
* Risk level: low.
* Recommended order: 3.
* Helpful comment notes:
  * State stays in `page.tsx` so sorting/filtering behavior remains unchanged.

### D. `LibraryBookCard`

* What JSX it owns:
  * cover-grid book card
  * cover image / no-cover placeholder
  * title link
  * small status/progress/date display
  * progress bar
* Expected props:
  * `row`
  * `stats`
...
* What stays in `page.tsx`:
  ...
  * progress and status data source
* Category: presentational UI.
* Risk level: low-medium.
* Recommended order: 4.
* Helpful comment notes:
  * This card should display already-loaded stats only; it should not calculate or fetch reading progress.
...
* Recommended order: 5.
* Helpful comment notes:
  * Pass `onOpen` from the page so navigation behavior stays centralized.
...
* Recommended order: 6.
* Helpful comment notes:
  * Keep grouping logic in `page.tsx`; this component should only render the supplied items.

### G. `LibraryEmptyState`

* What JSX it owns:
  * "No books yet." empty state
* Expected props:
  * optional `message`
* What stays in `page.tsx`:
  * `allValidRows.length === 0` decision
* Category: presentational UI.
* Risk level: low.
* Recommended order: 7.
...
* What JSX it owns:
  * shared visual shell for Ability Check reminder and super-teacher kanji reminder
  * title/body/buttons
* Expected props:
  * `tone`
  * `title`
  ...
  * `secondaryAction`
* What stays in `page.tsx`:
  * localStorage reminder behavior
  * due-card count logic
  * super-teacher visibility logic
  * routing callbacks
* Category: presentational UI with passed action callbacks.
* Risk level: medium.
* Recommended order: 8.
* Helpful comment notes:
  * This should only render a banner. It should not know why the banner is visible.

### I. `LearningTasksPanel`

* What JSX it owns:
  * learning tasks panel shell
  * learning task cards
  * task details text
  * action and mark-done button placement
* Expected props:
  * `tasks`
  * `rows`
  * `isViewingStudentLibrary`
  * `viewingLabel`
  * `currentUserCanComplete`
  * `completingLearningTaskId`
  * `getTaskTypeLabel`
  * `getTaskAction`
  * `onOpenTask`
  * `onCompleteTask`
* What stays in `page.tsx`:
  * task loading
  * task action derivation, at least for the first pass
  * completion handler
  * teacher/student visibility decisions
* Category: presentational UI with action/controller callbacks.
* Risk level: medium.
* Recommended order: 9.
* Helpful comment notes:
  * Keep task routing and completion in `page.tsx`; this component should receive already-decided actions.

### J. `PendingBookRequestsAlert`

* What JSX it owns:
  * pending book requests alert shell
  * request cards
  * approve/reject/dismiss button placement
* Expected props:
  * `bookRequests`
  * `onDismiss`
  * `onApprove`
  * `onReject`
* What stays in `page.tsx`:
  * super-teacher visibility
  * alert signature/dismissal logic
  * request loading
...
* Helpful comment notes:
  * This should not decide who can see or mutate book requests.

### K. `FloatingAddBookButton`

* What JSX it owns:
  * fixed `+ Add a Book` button
* Expected props:
  * `onClick`
  * optional `label`
* What stays in `page.tsx`:
  * whether the button should show for teacher/current user
  * navigation route
* Category: presentational UI.
* Risk level: low.
...
Start with `LibraryGuidePanel`.

It is the safest first extraction because it is mostly static visual JSX with simple route callbacks. It does not touch access checks, Supabase queries, role logic, viewed-user privacy, filtering, sorting, reading stats, learning-task completion, Ability Check timing, or book-request mutation.

After that, extract `LibraryHeader`, `LibraryViewControls`, and the simple book display components.
